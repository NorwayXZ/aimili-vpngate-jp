...
AimiliVPN JP 是一款基于官方 VPNGate 开放协议的高性能、零依赖 VPN 代理网关。它以纯 Python 标准库编写，内置美观响应式的管理网页，提供智能并发测速、多路由模式、出站代理网关、实时日志等强大功能。本版本仅拉取、测速和连接日本（JP）VPNGate 节点。

> 注意：日本节点数量完全取决于 VPNGate 官方公开列表中实时在线的日本 OpenVPN 志愿节点。如果官方当前没有日本节点，本项目会清空旧的不可用缓存并继续定时重试。
...
2. 点击 **“更新节点”** 按钮（或通过网页下方的网关/日志进行状态检查），程序会在后台通过多线程并发测速，自动筛选出延迟最低、可连接的日本 VPNGate 节点。
3. 选择您喜欢的出站路由模式：
...
AimiliVPN JP is a high-performance, zero-dependency VPN proxy gateway built entirely using Python's standard library. It parses official VPNGate servers, benchmarks latency, and routes traffic through a built-in dual-protocol (HTTP/SOCKS5) proxy server. This version only fetches, benchmarks, and connects to Japan (JP) VPNGate nodes.

> Note: JP node availability depends entirely on live Japan OpenVPN volunteer nodes in the official VPNGate list. If none are available, this project clears stale unavailable cache entries and keeps retrying.
...
2. Under "Admin", you can trigger node fetching. The backend concurrently tests official Japan VPNGate nodes and ranks them by latency.
